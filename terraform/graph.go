...
func (g *Graph) walk(walker GraphWalker) error {
	...
	walkFn = func(v dag.Vertex) (rerr error) {
		...
		if ev, ok := v.(GraphNodeEvalable); ok {
			...
			g.VertexDebugInfo(v, fmt.Sprintf("evaluating %T(%s)", v, path))
		...
		if ev, ok := v.(GraphNodeDynamicExpandable); ok {
			...
			g.VertexDebugInfo(v, fmt.Sprintf("expanding %T(%s)", v, path))
		...
		if sn, ok := v.(GraphNodeSubgraph); ok {
			...
			g.VertexDebugInfo(v, fmt.Sprintf("subgraph: %T(%s)", v, path))
